# ============================================================
# FAST v3 leakage-safe baseline for bound prediction
# ============================================================
#
# Main design changes vs v2
# ------------------------------------------------------------
# 1) Still NO per-renew_dt hidden walk-forward model training.
#    Hidden scoring trains at most ONE final model, or uses ONE prior.
#
# 2) No month/quarter/year grouped training anywhere.
#    Any groupby below is diagnostics, schema summaries, target-encoding
#    dictionary building, or metric reporting. None of those groupby calls
#    repeatedly fit models by month/quarter/date.
#
# 3) Replace "drop_duplicates(keep='last') + m:1 merge + availability gate"
#    with true per-row as-of attach:
#       for each answer row and source, pick the source row with matching _k
#       and source_dt <= renew_dt and maximum source_dt.
#    This is correct even if future versions of csad/smad contain multiple
#    snapshots per DUNS.
#
# 4) Do not treat year / month_index as linear numeric trends by default.
#    Use categorical renewal timing features instead:
#       renew_year_cat, renew_month_cat, renew_quarter_cat
#    Trend / base-rate drift enters through recent-prior and blend models.
#
# 5) Add leakage-safe, train-only target encoding for selected answer-side
#    categorical features. The encoding dictionary is fit only on train_idx;
#    validation / hidden rows use mapping + global train fallback.
#
# 6) Add small C grid for logistic models and bootstrap CI on validation
#    metrics. This adds a handful of fits, not a per-date training loop.
#
# 7) Print dropped_reason summaries so feature deletion is auditable.
#
# Not default in this baseline
# ------------------------------------------------------------
# - INCLUDE_EXTERNAL_PAYLOAD remains False by default. Turn it on only as a
#   separate experiment once the answer-only baseline is stable.
# - Expanding-window CV is not enabled here; bootstrap CI is used as the
#   lightweight uncertainty check. If you want 3-fold time CV later, add it as
#   a separate model-selection audit, not in hidden prediction.
#
